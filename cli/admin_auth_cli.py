...
def main_auth_management():
    ...
    while True:
        ...
        console.print("[bold cyan]1.[/bold cyan] Register user")
        console.print("[bold cyan]2.[/bold cyan] Log-in")
        console.print("[bold cyan]0.[/bold cyan] Back\n")
        
        
        choice = Prompt.ask("👉 Enter your option").strip()
        
        if choice == "1":
            while True:
                console.print(f"\n[green]Username must contain 8-12 characters, 1 uppercase, 1 lowercase, 1 number[/green]")
                console.print(f"\n[green]Password must contain at least 8-12 characters, 1 uppercase, 1 lowercase, 1 number[/green]\n")
                username = Prompt.ask("\nUsername")
                password = Prompt.ask("Password")
                
                if not username.strip() or not password.strip():
                    print_error("Username or password cannot be empty")
                else:
                    break
